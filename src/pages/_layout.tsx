import { Outlet, NavLink } from "react-router-dom"

type LayoutProps = { showHeader?: boolean }

const navLinkClass = ({ isActive }: { isActive: boolean }) =>
  `text-sm text-muted-foreground hover:text-foreground transition-colors ${isActive ? "text-foreground font-medium" : ""}`

export default function Layout({ showHeader = true }: LayoutProps) {
  return (
    <div className="min-h-dvh flex flex-col">
      {showHeader && (
        <header className="h-14 border-b flex items-center">
          <div className="mx-auto w-full max-w-7xl px-6 flex items-center justify-between">
            <span className="font-semibold">Contacts &amp; Web Roles</span>
            <nav className="flex items-center gap-6">
              <NavLink to="/contacts" className={navLinkClass}>Contacts</NavLink>
              <NavLink to="/web-roles" className={navLinkClass}>Web roles</NavLink>
            </nav>
          </div>
        </header>
      )}

      <main className="flex-1 flex">
        <div className="flex-1 mx-auto w-full max-w-7xl">
          <Outlet />
        </div>
      </main>
    </div>
  )
}